...
class OpenInclude(sublime_plugin.TextCommand):
        ...
        for path in paths:
            ...
            for extension in extensions:
                ...
                if not opened and view.file_name() != None and view.file_name() != '':
                    ...
                    opened = self.try_open(window, maybe_path, line)
                    if opened:
                        something_opened = True
                ...
                if not opened:
                    for maybe_path in sublime.active_window().folders():
                        ...
                        print ("%s%s") % (maybe_path_tpm, line)
                        print "now"
                        opened = self.try_open(window, maybe_path_tpm, line)
                        if opened:
                            something_opened = True
                            break
                ...
                # absolute
                if not opened:
                    opened = self.try_open(window, newpath, line)
                    if opened:
                        something_opened = True
    ...
    def try_open(self, window, maybe_path, line=None):
        print maybe_path
        ...
        if os.path.isfile(maybe_path):
            if BINARY.search(maybe_path):
                ...
                window.open_file(maybe_path + line, sublime.ENCODED_POSITION)
